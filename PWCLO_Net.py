...
def placeholder_inputs(batch_size, num_point):
    ...
    pointclouds_pl = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, num_point * 2, 3))

    q_gt = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, 4))
    t_gt = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, 3,1))
...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    with tf.compat.v1.variable_scope('sa1') as scope:
    ...
    W_l3_feat1 =  tf.nn.softmax(l3_cost_volume_w, axis=1)

    l3_points_f1_new = tf.reduce_sum(l3_points_predict*W_l3_feat1, axis = 1, keepdims = True)
    l3_points_f1_new_big = tf_util.conv1d(l3_points_f1_new, 256, 1, padding='VALID', activation_fn=None, scope='l3_big')
    
    l3_points_f1_new_q = tf.compat.v1.layers.dropout(l3_points_f1_new_big, rate = 0.5, training = is_training)
    l3_points_f1_new_t = tf.compat.v1.layers.dropout(l3_points_f1_new_big, rate = 0.5, training = is_training)

    l3_q_coarse = tf_util.conv1d(l3_points_f1_new_q, 4, 1, padding='VALID', activation_fn=None, scope='l3_q_coarse')
    l3_q_coarse = l3_q_coarse / (tf.sqrt(tf.reduce_sum(l3_q_coarse*l3_q_coarse, axis=-1, keepdims=True)+1e-10) + 1e-10)
    ...
    W_l2_cost_volume =  tf.nn.softmax(l2_cost_volume_w, axis=1)


    l2_cost_volume_sum = tf.reduce_sum(l2_cost_volume_predict * W_l2_cost_volume, axis = 1, keepdims = True)
    l2_cost_volume_sum_big = tf_util.conv1d(l2_cost_volume_sum, 256, 1, padding='VALID', activation_fn=None, scope='l2_big')
    
    l2_cost_volume_sum_q = tf.compat.v1.layers.dropout(l2_cost_volume_sum_big, rate = 0.5, training = is_training)
    l2_cost_volume_sum_t = tf.compat.v1.layers.dropout(l2_cost_volume_sum_big, rate = 0.5, training = is_training)


    l2_q_det = tf_util.conv1d(l2_cost_volume_sum_q, 4, 1, padding='VALID', activation_fn=None, scope='l2_q_det')
    l2_q_det = l2_q_det / (tf.sqrt(tf.reduce_sum(l2_q_det*l2_q_det, axis=-1, keepdims=True)+1e-10) + 1e-10)
    ...
    W_l1_cost_volume =  tf.nn.softmax(l1_cost_volume_w, axis=1)


    l1_cost_volume_8 = tf.reduce_sum(l1_cost_volume_predict*W_l1_cost_volume, axis = 1, keepdims = True)
    
    l1_cost_volume_sum_big = tf_util.conv1d(l1_cost_volume_8, 256, 1, padding='VALID', activation_fn=None, scope='l1_big')
    
    l1_cost_volume_sum_q = tf.compat.v1.layers.dropout(l1_cost_volume_sum_big, rate = 0.5, training = is_training)
    l1_cost_volume_sum_t = tf.compat.v1.layers.dropout(l1_cost_volume_sum_big, rate = 0.5, training = is_training)


    l1_q_det = tf_util.conv1d(l1_cost_volume_sum_q, 4, 1, padding='VALID', activation_fn=None, scope='l1_q_det')
    l1_q_det = l1_q_det / (tf.sqrt(tf.reduce_sum(l1_q_det*l1_q_det, axis=-1, keepdims=True)+1e-10) + 1e-10)
    ...
    W_l0_cost_volume =  tf.nn.softmax(l0_cost_volume_w, axis=1)


    l0_cost_volume_8 = tf.reduce_sum(l0_cost_volume_predict*W_l0_cost_volume, axis = 1, keepdims = True)
    
    l0_cost_volume_sum_big = tf_util.conv1d(l0_cost_volume_8, 256, 1, padding='VALID', activation_fn=None, scope='l0_big')
    
    l0_cost_volume_sum_q = tf.compat.v1.layers.dropout(l0_cost_volume_sum_big, rate = 0.5, training = is_training)
    l0_cost_volume_sum_t = tf.compat.v1.layers.dropout(l0_cost_volume_sum_big, rate = 0.5, training = is_training)


    l0_q_det = tf_util.conv1d(l0_cost_volume_sum_q, 4, 1, padding='VALID', activation_fn=None, scope='l0_q_det')
    l0_q_det = l0_q_det / (tf.sqrt(tf.reduce_sum(l0_q_det*l0_q_det, axis=-1, keepdims=True)+1e-10) + 1e-10)
    ...
    l0_q_norm = l0_q / (tf.sqrt(tf.reduce_sum(l0_q*l0_q, axis=-1, keepdims=True)+1e-10) + 1e-10)
    l1_q_norm = l1_q / (tf.sqrt(tf.reduce_sum(l1_q*l1_q, axis=-1, keepdims=True)+1e-10) + 1e-10)
    l2_q_norm = l2_q / (tf.sqrt(tf.reduce_sum(l2_q*l2_q, axis=-1, keepdims=True)+1e-10) + 1e-10)
    l3_q_norm = l3_q / (tf.sqrt(tf.reduce_sum(l3_q*l3_q, axis=-1, keepdims=True)+1e-10) + 1e-10)
...
def get_loss(l0_q, l0_t, l1_q, l1_t, l2_q, l2_t, l3_q, l3_t, qq_gt, t_gt, w_x, w_q):#####idx来选择真值
    ...
    l0_q_norm = l0_q / (tf.sqrt(tf.reduce_sum(l0_q*l0_q, axis=-1, keepdims=True)+1e-10) + 1e-10)
    l0_loss_q = tf.reduce_mean(tf.sqrt(tf.reduce_sum((qq_gt-l0_q_norm)*(qq_gt-l0_q_norm), axis=-1, keepdims=True)+1e-10)) 
    ...
    l1_q_norm = l1_q / (tf.sqrt(tf.reduce_sum(l1_q*l1_q, axis=-1, keepdims=True)+1e-10) + 1e-10)
    l1_loss_q = tf.reduce_mean(tf.sqrt(tf.reduce_sum((qq_gt-l1_q_norm)*(qq_gt-l1_q_norm), axis=-1, keepdims=True)+1e-10)) 
    ...
    l2_q_norm = l2_q / (tf.sqrt(tf.reduce_sum(l2_q*l2_q, axis=-1, keepdims=True)+1e-10) + 1e-10)
    l2_loss_q = tf.reduce_mean(tf.sqrt(tf.reduce_sum((qq_gt-l2_q_norm)*(qq_gt-l2_q_norm), axis=-1, keepdims=True)+1e-10))
    ...
    l3_q_norm = l3_q / (tf.sqrt(tf.reduce_sum(l3_q*l3_q, axis=-1, keepdims=True)+1e-10) + 1e-10)
    l3_loss_q = tf.reduce_mean(tf.sqrt(tf.reduce_sum((qq_gt-l3_q_norm)*(qq_gt-l3_q_norm), axis=-1, keepdims=True)+1e-10))
    ...
    tf.compat.v1.add_to_collection('losses', loss_sum)
